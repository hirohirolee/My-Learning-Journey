...
st.title('run_analysis.py - 自動化展示')
...
st.write(df[df['R&D Spend'] == 0].to_string())
st.write("\nZero values in Marketing Spend:")
st.write(df[df['Marketing Spend'] == 0].to_string())
...
st.write(f"Dropped {len(outliers)} outlier(s). Data shape went from {orig_shape} to {df_clean.shape}.")
...
st.write("Encoded dataset columns:", list(df_encoded.columns))
...
st.write(f"Multiple Linear Regression:")
st.write(f"  R2 Score: {lr_r2:.5f}")
st.write(f"  MAE: ${lr_mae:,.2f}")
st.write(f"Random Forest Regressor:")
st.write(f"  R2 Score: {rf_r2:.5f}")
...
st.write(lr_coef_df.to_string(index=False))
...
st.write(rf_imp_df.to_string(index=False))
...
st.write("--- Step 7: Done! Report generated successfully as 'Startup_Profit_Analysis_Report.md' ---")
