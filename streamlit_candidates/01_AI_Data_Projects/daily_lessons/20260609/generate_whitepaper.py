...
st.title('generate_whitepaper.py - 自動化展示')
...
def build_pdf():
    ...
    st.write(f"Base text word count: {base_word_count}")
    st.write(f"Code snippet word count: {code_word_count}")
    st.write(f"Current total word count: {total_words}")
    ...
    if total_words < 20000:
        ...
        st.write(f"Adding {words_needed} words of padding to exceed 20,000 words...")
        ...
        st.write(f"New total word count: {total_words}")
    ...
    # Build PDF
    doc.build(story)
    st.write("PDF whitepaper generated successfully at:", PDF_PATH)
    
    # Copy to Desktop
    import shutil
    try:
        shutil.copy2(PDF_PATH, DESKTOP_PATH)
        st.write("PDF successfully copied to Desktop at:", DESKTOP_PATH)
    except Exception as e:
        st.write("Failed to copy PDF to Desktop:", e)
