...
st.title('test_controller_run.py - 自動化展示')
...
st.write("Starting controller test...")
...
while controller._is_running:
    ...
    while not controller._metrics_queue.empty():
        st.write("Metric:", controller._metrics_queue.get())

st.write("Finished! Total results:", len(controller.results))
if controller.results:
    st.write("First post comments count:", len(controller.results[0].comments))
