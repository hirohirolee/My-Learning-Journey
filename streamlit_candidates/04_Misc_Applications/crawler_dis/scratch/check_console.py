...
st.title('check_console.py - 自動化展示')
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        page.on("console", lambda msg: st.write(f"[CONSOLE {msg.type}] {msg.text}"))
        page.on("pageerror", lambda err: st.write(f"[PAGEERROR] {err}"))
        page.on("requestfailed", lambda req: st.write(f"[REQ FAILED] {req.url} | {req.failure}"))
