...
st.title('inspect_small.py - 自動化展示')
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        info = await page.evaluate("""() => {
            const all = document.querySelectorAll('div, p, span, a, button');
            for (let e of all) {
                if (e.innerText && e.innerText.includes('充分運用') && e.children.length < 3) {
                    return {
                        tag: e.tagName,
                        html: e.outerHTML,
        ...
        st.write('Small banner info:', info)
        await browser.close()
