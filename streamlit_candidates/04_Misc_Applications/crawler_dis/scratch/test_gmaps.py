...
async def main():
    ...
    async with async_playwright() as p:
        ...
        st.write("Waiting for review tab to render...")
        try:
            ...
            st.write("Found review tab! Clicking...")
            await review_tab.click()
            clicked = True
        except Exception as e:
            st.write(f"Could not find review tab within 15s: {e}")
            clicked = False
            
        await page.wait_for_timeout(3000)
        await page.screenshot(path="scratch/step2_after_click.png")
        st.write("Saved step2_after_click.png")
        ...
        st.write("Saved step3_after_scroll.png")
        ...
        st.write("Saved page_text.txt")
        
        review_blocks = soup.find_all("div", class_="jJc9Ad")
        st.write(f"Found {len(review_blocks)} reviews.")
        ...
        st.write(f"Saved {len(comments)} comments to {output_file}")
        await browser.close()
