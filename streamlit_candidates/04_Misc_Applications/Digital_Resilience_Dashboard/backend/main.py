    ...
    async with ai_semaphore:
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout_settings) as client:
                ...
                st.write("[Mode: Local]")  # 本機成功時輸出 [Mode: Local]
...
async def call_gemini_fallback(prompt: str) -> str:
    ...
    try:
        ...
        st.write("[Mode: Cloud]")  # 切換雲端成功時輸出 [Mode: Cloud]
...
if __name__ == "__main__":
    st.write("---")
    st.subheader("函數測試區塊")
    if st.button("執行 anonymize"):
        try:
            res = anonymize() # 請視需要自行補上參數
            st.write("執行結果:", res)
        except Exception as e:
            st.error(f"執行出錯，可能需要提供參數：{e}")
    if st.button("執行 risk_assessment"):
        try:
            res = risk_assessment() # 請視需要自行補上參數
            st.write("執行結果:", res)
        except Exception as e:
            st.error(f"執行出錯，可能需要提供參數：{e}")
    if st.button("執行 retrieve_context"):
        try:
            res = retrieve_context() # 請視需要自行補上參數
            st.write("執行結果:", res)
        except Exception as e:
            st.error(f"執行出錯，可能需要提供參數：{e}")
    if st.button("執行 build_docx_report"):
        try:
            res = build_docx_report() # 請視需要自行補上參數
            st.write("執行結果:", res)
        except Exception as e:
            st.error(f"執行出錯，可能需要提供參數：{e}")
