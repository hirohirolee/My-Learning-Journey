...
def main():
    ...
    while True:
        st.write("\n請選擇選號模式 (大樂透 6/49)：")
        st.write("1: 完全隨機")
        st.write("2: 熱門加權 (依歷史出現頻率提高抽中機率)")
        st.write("3: 排除高遺漏值 (剔除近期最久未開出的號碼)")
        st.write("0: 結束程式")
        
        choice = st.text_input("請輸入模式代碼 (0-3): ").strip()
