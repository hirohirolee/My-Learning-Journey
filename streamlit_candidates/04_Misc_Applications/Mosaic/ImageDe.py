...
def main():
    ...
    if image_path == "sample_image.jpg":
        ...
        st.write(f"錯誤：找不到圖片 '{image_path}'！請確認路徑是否正確。")
        return
    ...
    # 訓練
    epochs = args.epochs
    st.write(f"開始針對圖片 '{image_path}' 進行訓練 (共 {epochs} Epochs)...")
    ...
    with torch.no_grad():
        ...
        st.write("\n訓練完成！結果已保存:")
        st.write("- result_lr.jpg (馬賽克/低畫質圖片)")
        st.write("- result_sr.jpg (AI 修復後圖片)")
        st.write("- result_hr.jpg (原始目標圖片)")
