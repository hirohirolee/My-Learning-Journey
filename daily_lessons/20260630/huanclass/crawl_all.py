...
def crawl_pages(max_pages=10):
    ...
    for page in range(1, max_pages + 1):
        ...
        parser.close()
        print(f"  Parsed {len(parser.movies)} movies from Page {page}.")
        ...
        for future in as_completed(futures):
            ...
            if success:
                if msg == "Already exists":
                    ...
                    print(f"  [{idx}/100] Downloaded poster for: {title}")
            else:
                print(f"  [{idx}/100] Failed poster download for {title}: {msg}", file=sys.stderr)
            results[idx] = success
            
    print("Poster downloads completed.")
...
def export_to_csv(movies, csv_path="movies.csv"):
    ...
    try:
        ...
        print(f"Failed to open primary CSV file {csv_path} (it may be open/locked): {e}", file=sys.stderr)
        f = None
        for i in range(1, 10):
            ...
            try:
                ...
                print(f"Will save CSV to fallback file: {csv_path}")
                break
            except Exception:
                continue
        if not f:
            print("Failed to save CSV file to any location.", file=sys.stderr)
            return
    ...
    try:
        ...
        print(f"Success! Saved 100 movies to CSV: {csv_path}")
    except Exception as e:
...
def export_to_excel(movies, excel_path="movies.xlsx"):
    ...
    try:
        wb.save(excel_path)
        print(f"Success! Saved 100 movies with embedded posters to Excel: {excel_path}")
    except Exception as e:
        print(f"Failed to save Excel file to {excel_path} (it may be open/locked): {e}", file=sys.stderr)
        # Try fallback names
        for i in range(1, 10):
            fallback_path = f"movies_{i}.xlsx"
            try:
                wb.save(fallback_path)
                print(f"Success! Saved to fallback file: {fallback_path}")
                break
