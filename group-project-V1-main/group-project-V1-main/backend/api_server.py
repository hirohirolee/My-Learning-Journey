...
from supabase_db import save_pr_report, supabase, SUPABASE_TABLE_NAME, SUPABASE_RESULT_TABLE_NAME, fetch_existing_result_ids, upsert_ml_analysis_result
...
def _dashboard_row_from_review(record: dict, index: int = 0, classify_content: bool = False) -> dict:
    ...
    if classify_content:
        row.update(_classify_content_quality_with_ollama(text, record))
    return row
...
def ml_dashboard_result_data(limit: int = 0):
    try:
        from supabase_db import fetch_result_reports
        rows = fetch_result_reports()
    except Exception as exc:
...
def supabase_query_compatible(table: str = "master_reviews_result", select: str = "*", limit: int = 0, order: str = "comment_published_at.desc.nullslast"):
    if table == SUPABASE_RESULT_TABLE_NAME or table == "master_reviews_result":
        return ml_dashboard_result_data(limit=limit)
    return ml_dashboard_data(limit=limit)



@app.post("/api/ml-dashboard/sync")
def sync_ml_dashboard_to_supabase(limit: int | None = None, dry_run: bool = False, force: bool = False):
    """Run ML dashboard analysis and upsert generated fields into master_reviews_result."""
    try:
        from supabase_db import fetch_all_reports
        records = fetch_all_reports()
    except Exception as exc:
    ...
    if not force:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to fetch existing result ids: {exc}")
    ...
    rows = [_dashboard_row_from_review(record, index, classify_content=True) for index, record in enumerate(selected_records)]
    _export_dashboard_csv(rows)
...
def resolve_review_api(req: ResolveReviewRequest):
    ...
    try:
        ...
        try:
            sample = supabase.table(SUPABASE_RESULT_TABLE_NAME).select("*").limit(1).execute()
...
def ai_reply_proxy(req: AiReplyProxyRequest):
    ...
    if provider not in {"openai", "gemini", "huggingface"}:
        raise HTTPException(status_code=400, detail="This proxy supports OpenAI, Gemini, and Hugging Face only.")
    if not model:
