...
if SUPABASE_URL and SUPABASE_KEY:
    ...
    supabase = None
    print("[Info] SUPABASE_URL and a Supabase key are required.")
...
def search_similar_reviews_hybrid(query_text, query_vector, match_threshold=0.70, limit=3):
    ...
    try:
        ...
        print(f"[Warning] Supabase match_reviews_hybrid RPC failed: {e}")
        return []


def fetch_all_reports(batch_size: int = 1000):
    if not supabase:
        return []
    try:
        data: list[dict] = []
...
def fetch_existing_result_ids(batch_size: int = 1000) -> set:
    ...
    try:
        ...
        print(f"[Warning] Failed to fetch existing result ids: {e}")
        raise e


def fetch_result_reports(batch_size: int = 1000):
    if not supabase:
        return []
    try:
        data: list[dict] = []
        ...
        print(f"[Warning] Failed to fetch result reports from Supabase: {e}")
        raise e
