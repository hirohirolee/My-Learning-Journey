...
async def run_single(
    ...
    print(f"\n✅ 執行完成！")
    if csv_path:
        print(f"   📊 Power BI CSV：{csv_path}")
    if md_path:
        print(f"   📋 分析報告：{md_path}")
...
async def run_status(client: OddsApiClient, db: OddsDatabase) -> None:
    ...
    print("  系統狀態摘要")
    print("═" * 50)
    print(f"  月份：        {quota['month_key']}")
    print(f"  月額度：      {quota['monthly_quota']} 次")
    print(f"  已用次數：    {quota['calls_this_month']} 次（{quota['usage_ratio_pct']:.1f}%）")
    print(f"  剩餘次數：    {quota['remaining_quota']} 次")
    print(f"  今日建議上限：{quota['daily_budget']} 次")
    print("─" * 50)
    print(f"  DB 總記錄：   {summary['total_records']:,} 筆")
    print(f"  有效記錄：    {summary['valid_records']:,} 筆")
    print(f"  無效記錄：    {summary['invalid_records']:,} 筆")
    print(f"  運動項目：    {', '.join(summary['sports']) or '(無資料)'}")
    print(f"  覆蓋平台：    {len(summary['platforms'])} 個")
    print(f"  最新資料：    {summary['latest_timestamp'] or '(無資料)'}")
    print("═" * 50)
