...
print("\n=== 所有店家爬取結束 ===")
print("總共抓取店家數:", len(controller.results))
for idx, res in enumerate(controller.results, 1):
    print(f"[{idx}] 店家標題: {res.title} | 成功爬取評論數: {len(res.comments)} 則")

# 驗證匯出檔案
df_comments = pd.read_csv("output/export_comments.csv")
print("\n匯出檔案 `output/export_comments.csv` 驗證:")
print("總留言筆數:", len(df_comments))
print("包含店家:", df_comments["post_title"].unique().tolist())
print("前 3 筆評論預覽:")
for _, row in df_comments.head(3).iterrows():
    print(f" - [{row['post_title']}] {row['author']}: {str(row['content'])[:40]}...")
