...
def enterPlayerMove(previousMoves):
    ...
    print('您要將下一個聲納設備投放在哪裡？(X座標 0-59 Y座標 0-14) (或輸入 quit 離開)')
    while True:
        move = input()
        if move.lower() == 'quit':
            print('遊戲結束！')
        ...
        print('輸入座標無效，請輸入一個數字（0-59），空一格，再輸入一個數字（0-14）。')
...
def main():
    ...
    while True:
        ...
        while sonarDevices > 0:
            ...
            print(f'您還有 {sonarDevices} 個聲納探測儀。海洋中還有 {len(theChests)} 個寶藏箱。')
            ...
            if len(theChests) == 0:
                print('太棒了！您找到了所有隱藏的寶藏！恭喜您！')
                break

            sonarDevices -= 1

        if sonarDevices == 0:
            print('我們耗盡了所有的聲納設備！現在只能轉向回航，無法找到剩下的寶藏了。')
            print('遊戲結束。')
            print('剩下的寶藏位於：')
