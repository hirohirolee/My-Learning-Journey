...
def import_iso_document(file_path: str) -> bool:
    ...
    if not os.path.exists(file_path):
        print(f"[WARNING] 找不到條文檔案：{file_path}")
        return False
    ...
    print(f"[OK] 成功匯入文件 {file_path}，共切為 {len(chunks)} 片段存入向量庫。")
    return True
...
def init_mock_data():
    ...
    try:
        ...
        if current_count >= EXPECTED_MIN_CHUNKS:
            print(f"[OK] 向量知識庫已就緒（現有 {current_count} 片段，門檻 {EXPECTED_MIN_CHUNKS}），跳過重建。")
            return

        # 若片段不足（首次啟動或知識庫升級後），清空後重新導入
        if current_count > 0:
            print(f"[INFO] 向量知識庫片段數 ({current_count}) 低於門檻，正在重建以套用最新 ESG 知識庫...")
        ...
        print(f"[OK] ESG 知識庫（ISO 14064-1 深度擴充版）已成功寫入向量資料庫。")

    except Exception as e:
        print(f"[WARNING] 初始化 Mock 向量資料庫時出錯: {e}")
