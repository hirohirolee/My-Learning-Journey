...
# ── Page Config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Boston Housing ML Studio",
    page_icon="🏠",
    layout="wide",
...
.metric-card h3 { color: #a78bfa; font-size: 0.85rem; margin: 0 0 6px; letter-spacing: 0.5px; }
...
@st.cache_data
def load_data():
    csv_path = Path(__file__).parent / "HousingData.csv"
...
# ── Main Title ────────────────────────────────────────────────────────────────
st.title("🏠 Boston Housing — ML Studio")
...
with st.spinner("⏳ 訓練中，請稍候..."):
    ...
    X_train_sc_df = pd.DataFrame(X_train_sc, columns=X.columns, index=X_train.index)
    X_test_sc_df  = pd.DataFrame(X_test_sc,  columns=X.columns, index=X_test.index)

    # SelectKBest for active model
    ...
    # ── Benchmark (8 selectors × all K) ──────────────────────────────────────
    pearson_sc  = [np.abs(np.corrcoef(X_train_sc[:, i], y_train)[0, 1]) for i in range(total_features)]
    spearman_sc = [np.abs(spearmanr(X_train_sc[:, i], y_train).correlation) for i in range(total_features)]
    f_sc, _     = f_regression(X_train_sc, y_train)
    mi_sc       = mutual_info_regression(X_train_sc, y_train, random_state=42)

    rfe = RFE(estimator=LinearRegression(), n_features_to_select=1)
    rfe.fit(X_train_sc, y_train)

    lasso_cv  = LassoCV(cv=3, random_state=42).fit(X_train_sc, y_train)
    rf_bench  = RandomForestRegressor(n_estimators=30, random_state=42, n_jobs=-1).fit(X_train_sc, y_train)
    ...
    while sfs_remaining:
        ...
        for f in sfs_remaining:
            s = np.mean(cross_val_score(LinearRegression(), X_train_sc[:, sfs_selected + [f]], y_train, cv=3, scoring="r2"))
            if s > best_s:
                best_s, best_f = s, f
...
with tab1:
    ...
    c1.metric("🏋️ Train RMSE", f"{train_rmse:.4f}")
    ...
    fig_sc.add_trace(go.Scatter(
        x=y_test.values, y=pred_test, mode="markers",
        marker=dict(color="#6366f1", opacity=0.65, size=6),
        name="預測點"
    ))
    ...
    fig_sc.update_layout(
        ...
        xaxis_title="實際值 (MEDV)", yaxis_title="預測值 (MEDV)",
...
with tab2:
    st.subheader("特徵重要性分析（Mutual Info 選出的特徵）")
    st.markdown(f"**選中特徵（K={k_real}）：** {', '.join(selected_cols)}")

    imp_sorted = sorted(imp.items(), key=lambda x: x[1], reverse=True)
    feat_names = [i[0] for i in imp_sorted]
    feat_vals  = [i[1] for i in imp_sorted]
    ...
    fig_imp.update_layout(
        template="plotly_dark", height=max(300, 35 * len(feat_names)),
