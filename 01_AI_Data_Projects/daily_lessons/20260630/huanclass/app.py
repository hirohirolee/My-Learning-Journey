...
def load_movies_from_csv():
    ...
    if not os.path.exists(csv_path):
        print(f"WARNING: '{csv_path}' not found on startup! Please run crawl_all.py to generate it.")
        return
    ...
    try:
        ...
        print(f"Successfully loaded {len(movies_db)} movies from CSV.")
    except Exception as e:
        print(f"ERROR: Failed to load movies from CSV: {e}")
...
def chat_bot(req: ChatRequest):
    ...
    if provider == "gemini":
        ...
        try:
            ...
            print(f"OpenAI-Compatible API Error: {e}", file=sys.stderr)
