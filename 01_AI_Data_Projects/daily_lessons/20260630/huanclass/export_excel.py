...
def export_to_excel():
    ...
    if not os.path.exists(html_path):
        print(f"Error: '{html_path}' does not exist. Please run crawler.py first.")
        return
    ...
    print(f"Exporting movies to Excel with embedded posters...")
    ...
    for idx, movie in enumerate(movies, 1):
        ...
        if os.path.exists(local_path):
            try:
                ...
                print(f"  Error inserting image for row {idx}: {e}")
    ...
    try:
        wb.save(excel_path)
        print(f"Success! Generated Excel spreadsheet: {excel_path}")
    except Exception as e:
